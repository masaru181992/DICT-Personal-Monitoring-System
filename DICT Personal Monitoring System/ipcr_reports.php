...
$sort_direction = strtoupper($_GET['order'] ?? 'DESC');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle saving IPCR Template separately
    if (isset($_POST['action']) && $_POST['action'] === 'save_template') {
        try {
            $pdo->beginTransaction();
            
            $template_name = isset($_POST['template_name']) ? trim($_POST['template_name']) : '';
            $template_description = isset($_POST['template_description']) ? trim($_POST['template_description']) : null;
            
            // Validate allowed values
            $allowed_names = ['Core Function', 'Support Function'];
            if (!in_array($template_name, $allowed_names, true)) {
                throw new Exception('Invalid template name');
            }
            
            // Insert template (payload left NULL for now)
            $stmt = $pdo->prepare("INSERT INTO ipcr_templates (user_id, template_name, description, payload) VALUES (:user_id, :template_name, :description, NULL)");
            $stmt->execute([
            ...
            $pdo->commit();
            $_SESSION['success'] = 'IPCR template saved successfully';
        } catch (Exception $e) {
            if ($pdo->inTransaction()) {
                $pdo->rollBack();
            }
            error_log('Error saving IPCR template: ' . $e->getMessage());
            $_SESSION['error'] = 'Failed to save template: ' . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8');
        }
        header('Location: ipcr_reports.php?success=1');
        exit();
    }
    ...
    try {
        ...
        $success_indicators_quantity = isset($_POST['success_indicators_quantity']) ? (int)$_POST['success_indicators_quantity'] : 1;
        $actual_accomplishments = isset($_POST['actual_accomplishments']) ? trim($_POST['actual_accomplishments']) : '';
        
        // Debug log all POST data
        error_log('POST data: ' . print_r($_POST, true));
        
        // Get quantities from both edit and add forms
        if (isset($_POST['editSuccessIndicatorsQuantity'])) {
            $success_indicators_quantity = (int)$_POST['editSuccessIndicatorsQuantity'];
            error_log('Using editSuccessIndicatorsQuantity: ' . $success_indicators_quantity);
        } elseif (isset($_POST['success_indicators_quantity'])) {
            $success_indicators_quantity = (int)$_POST['success_indicators_quantity'];
            error_log('Using success_indicators_quantity: ' . $success_indicators_quantity);
        } else {
            $success_indicators_quantity = 1; // Default value
            error_log('Using default success_indicators_quantity: ' . $success_indicators_quantity);
        }
        
        if (isset($_POST['editActualAccomplishmentsQuantity'])) {
            $actual_accomplishments_quantity = (int)$_POST['editActualAccomplishmentsQuantity'];
            error_log('Using editActualAccomplishmentsQuantity: ' . $actual_accomplishments_quantity);
        } elseif (isset($_POST['actual_accomplishments_quantity'])) {
            $actual_accomplishments_quantity = (int)$_POST['actual_accomplishments_quantity'];
            error_log('Using actual_accomplishments_quantity: ' . $actual_accomplishments_quantity);
        } else {
            $actual_accomplishments_quantity = 0; // Default value for new entries
            error_log('Using default actual_accomplishments_quantity: ' . $actual_accomplishments_quantity);
        }
        ...
        error_log('Success Indicators Qty: ' . $success_indicators_quantity);
        error_log('Actual Accomplishments Qty: ' . $actual_accomplishments_quantity);
        
        // Validate required fields
        if (empty($year) || empty($semester) || empty($function_type) || empty($success_indicators)) {
            throw new Exception('All fields are required');
        }
        
        // Set default values for removed fields
        $actual_accomplishments = $actual_accomplishments ?: 'N/A';
        // Ensure actual_accomplishments_quantity is at least 0
        $actual_accomplishments_quantity = max(0, (int)$actual_accomplishments_quantity);
        
        if ($entry_id > 0) {
            // Update existing entry
            $sql = "UPDATE ipcr_entries 
                   SET year = :year, 
                       semester = :semester, 
                       function_type = :function_type, 
                       success_indicators = :success_indicators,
                       success_indicators_quantity = :success_indicators_quantity,
                       actual_accomplishments = :actual_accomplishments,
                       actual_accomplishments_quantity = :actual_accomplishments_quantity,
                       updated_at = NOW() 
            ...
            // Add new entry
            $sql = "INSERT INTO ipcr_entries 
                (user_id, year, semester, function_type, success_indicators, success_indicators_quantity, actual_accomplishments, actual_accomplishments_quantity) 
                VALUES (:user_id, :year, :semester, :function_type, :success_indicators, :success_indicators_quantity, :actual_accomplishments, :actual_accomplishments_quantity)";
            ...
            try {
                ...
                error_log('New entry added with ID: ' . $entry_id);
            } catch (PDOException $e) {
                error_log('Error adding new entry: ' . $e->getMessage());
                throw $e;
            }
        }
        ...
        $_SESSION['error'] = 'An error occurred: ' . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8');
    }
...
try {
    ...
    // Prepare base query with proper parameter binding
    $query = "SELECT * FROM ipcr_entries WHERE user_id = :user_id ORDER BY ";
    
    // Add sorting
    $valid_columns = ['year', 'semester', 'function_type', 'success_indicators', 'created_at'];
    ...
    if (!$stmt->execute()) {
        throw new Exception('Failed to fetch IPCR entries');
    }
    
    $entries = $stmt->fetchAll(PDO::FETCH_ASSOC);
    // First, check if the columns exist in the table
    $stmt = $pdo->prepare("SHOW COLUMNS FROM ipcr_entries LIKE 'success_indicators_quantity'");
    $stmt->execute();
    $hasSuccessIndicatorsQty = $stmt->rowCount() > 0;
    
    $stmt = $pdo->prepare("SHOW COLUMNS FROM ipcr_entries LIKE 'actual_accomplishments_quantity'");
    $stmt->execute();
    $hasActualAccomplishmentsQty = $stmt->rowCount() > 0;
    
    // Log the column check results
    error_log('success_indicators_quantity exists: ' . ($hasSuccessIndicatorsQty ? 'yes' : 'no'));
    error_log('actual_accomplishments_quantity exists: ' . ($hasActualAccomplishmentsQty ? 'yes' : 'no'));
    
    // Explicitly list all columns to ensure we get them
    $query = "SELECT id, user_id, year, semester, function_type, success_indicators, " .
             ($hasSuccessIndicatorsQty ? "success_indicators_quantity, " : "1 as success_indicators_quantity, ") .
             ($hasActualAccomplishmentsQty ? "actual_accomplishments_quantity, " : "0 as actual_accomplishments_quantity, ") .
             "actual_accomplishments, created_at, updated_at " .
             "FROM ipcr_entries WHERE user_id = :user_id " .
             "ORDER BY created_at DESC, year DESC, FIELD(semester, '2nd', '1st') DESC";
    ...
    // Log the first entry to verify data
    if (!empty($entries)) {
        error_log('First entry data: ' . print_r($entries[0], true));
    }
} catch (PDOException $e) {
...
// Add sorting to the query - default to showing latest first
if ($sort_column === 'created_at') {
    $query .= " ORDER BY created_at DESC, year DESC, FIELD(semester, '2nd', '1st') DESC";
} else {
    $query .= " ORDER BY $sort_column $sort_direction, created_at DESC";
}
...
<head>
    ...
    <title>The Personal Monitoring System - IPCR Reports</title>
...
<body>
    <div class="container-fluid">
        <div class="row h-100">
            ...
            <!-- Main Content -->
            <div class="main-content animate__animated animate__fadeIn" style="margin-left: 350px; padding: 20px; width: calc(100% - 350px);">
                <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap">
                    <div class="mb-3 mb-md-0">
                    <h4 class="mb-0" style="font-weight: 700; letter-spacing: 1px; color: var(--accent-color); font-size: 1.5rem; text-transform: uppercase;">IPCR Reports</h4>
                        <p class="text-white mb-0">View and manage your IPCR reports</p>
                    </div>
                    ...
                    <div class="card-body">
                        <form id="filterForm" method="GET" class="row g-3">
                            ...
                            <div class="col-md-3 d-flex align-items-end">
                                ...
                                <a href="ipcr_reports.php" class="btn btn-custom">
                ...
                <div class="card animate__animated animate__fadeInUp">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-custom" data-bs-toggle="modal" data-bs-target="#addIpcrtModal" 
                                    title="Add a new IPCR entry">
                                <i class="bi bi-plus-lg me-2"></i>Add New Entry
                            </button>
                            <button type="button" class="btn btn-custom" data-bs-toggle="modal" data-bs-target="#addIpcrTemplateModal"
                                    title="Create an IPCR template you can reuse later">
                                <i class="bi bi-file-earmark-plus me-2"></i>Add IPCR Template
                            </button>
                            <button type="button" class="btn btn-custom" data-bs-toggle="tooltip" 
                            ...
                            <table class="table table-hover mb-0" style="width: 100%; min-width: 1200px;">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width: 300px; min-width: 250px;"><?php echo getSortLink('success_indicators', 'Description Indicator'); ?></th>
                                        <th style="width: 100px;" class="text-center"><?php echo getSortLink('success_indicators_quantity', 'Target Qty'); ?></th>
                                        <th style="width: 100px;" class="text-center">Actual Qty</th>
                                ...
                                <tbody>
                                    <?php foreach ($entries as $entry): 
                                        ...
                                        error_log('Entry ID: ' . $entry['id'] . 
                                                ', success_indicators_quantity: ' . ($entry['success_indicators_quantity'] ?? 'not set') .
                                                ', actual_accomplishments_quantity: ' . ($entry['actual_accomplishments_quantity'] ?? 'not set'));
                                    ?>
                                        <tr data-entry-id="<?php echo $entry['id']; ?>"
                                            data-success-indicators-quantity="<?php echo $entry['success_indicators_quantity'] ?? '1'; ?>"
                                            data-actual-accomplishments-quantity="<?php echo $entry['actual_accomplishments_quantity'] ?? '1'; ?>">
                                            <td><?php echo $entry['year']; ?></td>
                                            <td><?php echo $entry['semester']; ?></td>
                                            <td><?php echo $entry['function_type']; ?></td>
                                            <td class="text-truncate" title="<?php echo $entry['success_indicators']; ?>">
                                                <?php echo $entry['success_indicators']; ?>
                                            </td>
                                            <td class="text-center">
                                                <?php echo isset($entry['success_indicators_quantity']) ? $entry['success_indicators_quantity'] : '1'; ?>
                                            </td>
                                            <td class="text-center">
                                                <?php echo isset($entry['actual_accomplishments_quantity']) ? $entry['actual_accomplishments_quantity'] : '0'; ?>
                                            </td>
                                            <td>
                                                <button class="btn btn-sm btn-outline-primary me-1" title="Edit" onclick="handleEditClick(this)">
                                                    <i class="bi bi-pencil"></i>
                                                </button>
                                                <button class="btn btn-sm btn-outline-secondary me-1" title="Copy" onclick="handleCopyClick(this)">
                    ...
                    <div class="card-footer bg-transparent border-top">
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="small text-white-50">
                                ...
                                $start = 1;
                                $end = $total_entries;
                                echo "Showing <span class='fw-bold text-white'>{$start}</span> to <span class='fw-bold text-white'>{$end}</span> of <span class='fw-bold text-white'>{$total_entries}</span> entries";
                                ?>
                            </div>
                ...
                <form method="POST" action="">
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                <label for="success_indicators" class="form-label">Description Indicator</label>
                            ...
                            <div class="col-md-6">
                                <label for="success_indicators_quantity" class="form-label">Target Quantity</label>
                                <input type="number" class="form-control bg-dark text-white border-accent" 
                                       id="success_indicators_quantity" name="success_indicators_quantity" 
    ...
    <!-- Add IPCR Template Modal -->
    <div class="modal fade" id="addIpcrTemplateModal" tabindex="-1" aria-labelledby="addIpcrTemplateModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md">
            <div class="modal-content bg-dark border border-accent">
                <div class="modal-header border-bottom border-accent">
                    <h5 class="modal-title" id="addIpcrTemplateModalLabel">Add IPCR Template</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="POST" action="">
                    <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
                    ...
                    <div class="modal-body">
                        <div class="alert alert-info small" role="alert">
                            Create a reusable IPCR template you can apply when adding entries.
                        </div>
                        <div class="mb-3">
                            <label for="templateName" class="form-label">Template Name</label>
                            <select id="templateName" name="template_name" class="form-select bg-dark text-white border-accent">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="templateDescription" name="template_description" class="form-control bg-dark text-white border-accent" rows="3" placeholder="Describe what this template includes"></textarea>
                        </div>
                    </div>
                    <div class="modal-footer border-top border-accent">
                ...
                <form method="POST" action="" id="editIpcrtForm">
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                <label for="editSuccessIndicators" class="form-label">Description Indicator</label>
                                <textarea class="form-control bg-dark text-white border-accent" id="editSuccessIndicators" 
                                          name="success_indicators" rows="2" required></textarea>
                            </div>
                            <div class="col-md-6">
                                <label for="editSuccessIndicatorsQuantity" class="form-label">Target Quantity</label>
                                <input type="number" class="form-control bg-dark text-white border-accent" 
                                       id="editSuccessIndicatorsQuantity" name="success_indicators_quantity" 
                                       min="1" value="1" required>
                            </div>
                            <div class="col-md-6">
                                <label for="editActualAccomplishmentsQuantity" class="form-label">Quantity of Actual Accomplishment</label>
                                <input type="number" class="form-control bg-dark text-white border-accent" 
                                       id="editActualAccomplishmentsQuantity" name="actual_accomplishments_quantity" 
                                       min="0" value="0">
                            </div>
    ...
    <script>
        ...
        // Set default quantity values when Add IPCR Entry modal is shown
        document.getElementById('addIpcrtModal').addEventListener('show.bs.modal', function () {
            document.getElementById('success_indicators_quantity').value = '1';
        });
        ...
        // Show checked items in a modal and update quantity field
        function showCheckedItems() {
            ...
            // Update the actual accomplishments quantity field with the count of checked items
            const quantityField = document.getElementById('actual_accomplishments_quantity');
            if (quantityField) {
                quantityField.value = checkedItems.length > 0 ? checkedItems.length : '1';
            }
        ...
        function handleCopyClick(button) {
            ...
            const semester = row.cells[1].textContent.trim().includes('1st') ? '1st' : '2nd';
            const functionType = row.cells[2].textContent.trim();
            const successIndicators = row.cells[3].textContent.trim();
            const targetQuantity = row.getAttribute('data-success-indicators-quantity') || '1';
            
            // Get the add form elements
            const addForm = document.querySelector('#addIpcrtModal form');
            
            // Fill the add form with the copied data
            addForm.querySelector('[name="year"]').value = year;
            addForm.querySelector('[name="semester"]').value = semester;
            addForm.querySelector('[name="function_type"]').value = functionType;
            addForm.querySelector('[name="success_indicators"]').value = successIndicators;
            addForm.querySelector('[name="success_indicators_quantity"]').value = targetQuantity;
            
            // Clear any existing entry ID to ensure we're creating a new entry
            if (addForm.querySelector('[name="entry_id"]')) {
                addForm.querySelector('[name="entry_id"]').remove();
            }
            
            // Show the add modal using Bootstrap 5 modal
            const modal = new bootstrap.Modal(document.getElementById('addIpcrtModal'));
            modal.show();
        }
        ...
        function handleEditClick(button) {
            ...
            const functionType = row.cells[2].textContent.trim();
            const successIndicators = row.cells[3].textContent.trim();
            const successIndicatorsQuantity = row.getAttribute('data-success-indicators-quantity') || '1';
            const actualAccomplishmentsQuantity = row.getAttribute('data-actual-accomplishments-quantity') || '1';
            ...
            document.getElementById('editSuccessIndicatorsQuantity').value = successIndicatorsQuantity;
            document.getElementById('editActualAccomplishmentsQuantity').value = actualAccomplishmentsQuantity;
        ...
        function setupActivityCheckboxes() {
            ...
            window.updateSelectedActivities = function() {
                ...
                selectedCheckboxes.forEach(checkbox => {
                    const id = checkbox.id.replace('activity_', '');
                    selectedHtml += `
                ...
                // Update quantity field with the count of checked items
                if (quantityField) {
                    quantityField.value = selectedCheckboxes.length > 0 ? selectedCheckboxes.length : '0';
        ...
        function collectFormData(form) {
            ...
            for (let i = 0; i < elements.length; i++) {
                ...
                if (element.type === 'checkbox' || element.type === 'radio') {
                    ...
                    if (name.endsWith('[]')) {
                        ...
                        data[baseName].push(element.value);
                    } else {
                        // Handle single checkbox/radio
                    ...
                    if (name.endsWith('[]')) {
                        const baseName = name.slice(0, -2);
                        data[baseName] = selected;
                    } else {
                        data[name] = selected;
                    }
                } else if (element.type === 'number' || element.type === 'text' || element.type === 'hidden' || element.type === 'textarea') {
                    // Handle text, number, hidden, and textarea inputs
                    if (name.endsWith('[]')) {
                        const baseName = name.slice(0, -2);
                        if (!data[baseName]) {
                            data[baseName] = [];
            ...
            if (!data.success_indicators_quantity) {
                data.success_indicators_quantity = document.getElementById('editSuccessIndicatorsQuantity').value || '1';
            }
            // Ensure actual_accomplishments_quantity is properly set
            const actualAccomplishmentsQty = formData.get('actual_accomplishments_quantity') || '0';
            formData.set('actual_accomplishments_quantity', actualAccomplishmentsQty);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const editForm = document.querySelector('#editIpcrtForm');
            if (editForm) {
                editForm.addEventListener('submit', async function(e) {
                    ...
                    // Ensure entry_id is included in the form data
                    const entryId = document.getElementById('editEntryId').value;
                    if (!entryId) {
                        console.error('Entry ID is missing');
                        showToast('Error', 'Entry ID is missing', 'error');
                        return;
                    }
                    ...
                    try {
                        // Create form data and append all form fields
                        const formData = new FormData(form);
                        
                        // Add CSRF token
                        formData.append('csrf_token', '<?php echo $_SESSION['csrf_token']; ?>');
                        
                        // Get the entry ID and set required fields for the API
                        const entryId = document.getElementById('editEntryId').value;
                        if (entryId) {
                            formData.set('target_id', entryId);
                            ...
                            // Set the title field using the success_indicators value
                            const successIndicators = formData.get('success_indicators');
                            if (successIndicators) {
                                formData.set('title', successIndicators.substring(0, 255)); // Truncate to 255 chars if needed
                            }
                            
                            // Set target_quantity from success_indicators_quantity
                            const targetQty = formData.get('success_indicators_quantity') || '1';
                            formData.set('target_quantity', targetQty);
                            formData.set('success_indicators_quantity', targetQty); // Ensure the field is set for the update
                            
                            // Ensure actual_accomplishments_quantity is properly set
                            const actualAccomplishmentsQty = formData.get('actual_accomplishments_quantity') || '0';
                            formData.set('actual_accomplishments_quantity', actualAccomplishmentsQty);
                            
                            // Get all selected activities and add them to formData
                            const selectedActivities = [];
                            document.querySelectorAll('.activity-checkbox:checked').forEach(checkbox => {
                                selectedActivities.push(checkbox.value);
                            });
                            
                            // Remove any existing activities[] entries to avoid duplicates
                            formData.delete('activities[]');
                            
                            // Add each selected activity to formData
                            selectedActivities.forEach(activityId => {
                                formData.append('activities[]', activityId);
                            });
                            
                            // Ensure function_type is properly set
                            const functionType = formData.get('function_type');
                            if (!functionType) {
                                // Set default function type if not provided
                                formData.set('function_type', 'Core Function');
                            }
                            
                            // Set other required fields with default values if not present
                            if (!formData.has('category_id')) formData.set('category_id', '1');
                            if (!formData.has('status')) formData.set('status', 'Not Started');
                            if (!formData.has('priority')) formData.set('priority', 'Medium');
                            if (!formData.has('target_date')) {
                                const currentDate = new Date();
                                currentDate.setFullYear(currentDate.getFullYear() + 1); // Default to 1 year from now
                                formData.set('target_date', currentDate.toISOString().split('T')[0]);
                            }
                        }
                        
                        // Determine if this is an update or add operation
                        const url = entryId ? 'update_ipcr_target.php' : 'add_ipcr_target.php';
                        ...
                        if (result.success) {
                            ...
                            showToast('Success', result.message || 'IPCR entry saved successfully', 'success');
                            
                            // Close the modal after a short delay
                            setTimeout(() => {
                                const modal = bootstrap.Modal.getInstance(document.getElementById('editIpcrtModal'));
                                if (modal) modal.hide();
                            ...
                            throw new Error(result.message || 'Failed to save IPCR entry');
                        }
                    } catch (error) {
                        console.error('Error:', error);
                        showToast('Error', error.message || 'An error occurred while saving the IPCR entry', 'error');
                    } finally {
...
// Tooltip and sidebar functionality is handled in the main script section
</script>
</body>
</html>
