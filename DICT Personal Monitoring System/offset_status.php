...
try {
    ...
    try {
        ...
        // Get used offsets (from overtime requests and offset requests)
        $used_offsets_query = $pdo->prepare("
            SELECT 
                COALESCE(SUM(CASE 
                    WHEN orq.used_days IS NOT NULL THEN orq.used_days 
                    ELSE 0 
                END), 0) as total_used_days,
                (SELECT COALESCE(SUM(total_days), 0) 
                 FROM offset_requests 
                 WHERE user_id = ? AND status = 'approved') as total_offset_days
            FROM overtime_requests orq
            WHERE orq.user_id = ? AND orq.status = 'approved' AND orq.used_days > 0
        ");
        $used_offsets_query->execute([$user_id, $user_id]);
        ...
        $used_offsets = $used_offsets_data ? 
            (int)$used_offsets_data['total_used_days'] + (int)$used_offsets_data['total_offset_days'] : 0;
...
<body>
        ...
        <div class="row">
            ...
            <div class="col-md-10 ms-auto p-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h4 class="mb-0" style="font-weight: 700; letter-spacing: 1px; color: var(--accent-color); font-size: 1.5rem; text-transform: uppercase;">Overtime & Offset Management</h4>
                        <p class="text-white mb-0">View and manage your overtime and offset requests</p>
                    </div>
                    <div>
                        <a href="export_overtime_offset_report.php" class="btn btn-sm btn-custom">
                ...
                <div class="row g-4 mb-5">
                    ...
                    $max_offset_days = 24; // Maximum offset days allowed (2 days per month * 12 months)
                    $max_overtime_days = 5; // Maximum overtime days to convert to offset (40 hours / 8 hours per day)
                    
                    // Calculate trends (simplified - in a real app, you'd compare with previous period)
                    $offset_trend = $remaining_offset > ($max_offset_days / 2) ? 'up' : 'down';
                    $overtime_trend = $remaining_overtime > (($max_overtime_days * 8) / 2) ? 'up' : 'down';
                    
                    // Define cards data with real database values
                    $cards = [
                        [
                            'title' => 'Overtime Balance',
                            'value' => round($remaining_overtime / 8), // Convert hours to days (8-hour workday)
                            'subtitle' => 'days',
                            'icon' => 'stopwatch',
                            'bg' => 'info',
                            'details' => []
                    ...
                    foreach ($cards as $card): 
                        ...
                        $card['trend_value'] = $card['trend_value'] ?? '';
                        
                        // Only set trend icon and class if trend is not empty
                        $trend_icon = '';
                        $trend_class = '';
                        if (!empty($card['trend'])) {
                            $trend_icon = $card['trend'] === 'up' ? 'bi-arrow-up' : 'bi-arrow-down';
                            $trend_class = $card['trend'] === 'up' ? 'text-success' : 'text-danger';
                        }
                    ?>
                    <div class="col-12 col-md-6 col-lg-4">
                        <div class="analytics-card bg-white rounded-4 shadow-sm h-100 position-relative overflow-hidden transition-all" 
                             style="--card-bg: var(--bs-<?= $card['bg'] ?>-bg-subtle); --card-border: var(--bs-<?= $card['bg'] ?>-border-subtle);">
                            <!-- Glow effect -->
                            <div class="position-absolute top-0 end-0" style="width: 100px; height: 100px; background: radial-gradient(circle, rgba(var(--bs-<?= $card['bg'] ?>-rgb), 0.15) 0%, rgba(var(--bs-<?= $card['bg'] ?>-rgb), 0) 70%); transform: translate(30%, -30%);"></div>
                            
                            <div class="p-3 h-100 d-flex flex-column">
                                <!-- Header with title and icon -->
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <h6 class="mb-0 text-uppercase fw-semibold text-dark" style="font-size: 0.7rem; letter-spacing: 0.5px;">
                                        <?= htmlspecialchars($card['title']) ?>
                                    </h6>
                                    <div class="bg-<?= $card['bg'] ?>-subtle text-<?= $card['bg'] ?> rounded-circle p-2 d-flex align-items-center justify-content-center" style="width: 36px; height: 36px;">
                                        <i class="bi bi-<?= $card['icon'] ?>" style="font-size: 1.25rem;"></i>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex flex-column align-items-center text-center mb-3">
                                    <div class="mb-1">
                                        <h1 class="display-4 fw-bold text-dark mb-0" style="font-size: 3.5rem; line-height: 1.2;">
                                            <?= $card['value'] ?>
                                        </h1>
                                    </div>
                                    <div class="text-dark mb-2" style="font-size: 1.1rem;">
                                        <?= $card['subtitle'] ?>
                                    </div>
                                    <?php if (!empty($card['trend_value'])): ?>
                                    <div class="d-flex align-items-center mt-2 <?= $trend_class ?>">
                                        <?php if (!empty($trend_icon)): ?>
                                        <i class="bi <?= $trend_icon ?> me-1"></i>
                                        <?php endif; ?>
                                        <small class="fw-semibold"><?= $card['trend_value'] ?></small>
                                    </div>
                                    <?php endif; ?>
                                </div>
                                ...
                                <div class="mt-3 pt-2 border-top">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <div class="d-flex align-items-center small <?= $detail['class'] ?>">
                                            <i class="bi bi-<?= $detail['icon'] ?> me-1"></i>
                    ...
                    <div class="col-lg-6">
                        <div class="card request-card shadow-sm">
                            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                                ...
                                <button class="btn btn-sm btn-custom" data-bs-toggle="modal" data-bs-target="#addOffsetModal">
                    ...
                    <div class="col-lg-6">
                        <div class="card request-card shadow-sm">
                            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                                ...
                                <button class="btn btn-custom" data-bs-toggle="modal" data-bs-target="#addOvertimeModal">
                                    <i class="bi bi-plus-lg me-1"></i> Add Request
                                </button>
                            </div>
                            <div class="card-body">
                                        ...
                                        <table class="table table-hover request-table">
                                            ...
                                            <tbody>
                                                <?php foreach ($overtime_requests as $request): 
                                                    ...
                                                    $start_date = !empty($request['activity_start']) ? new DateTime($request['activity_start']) : new DateTime();
                                                    $end_date = !empty($request['activity_end']) ? new DateTime($request['activity_end']) : new DateTime();
    ...
    <script>
        ...
        // Function to update max days based on selected activity
        function updateMaxDays(activitySelect) {
            ...
            if (activityId && activitiesData[activityId]) {
                ...
                const helpText = daysInput.nextElementSibling;
                if (helpText && helpText.classList.contains('form-text')) {
                    helpText.textContent = 'Select an activity first to see the maximum allowed days';
                }
            }
        }

        // Initialize the modal event listener when the document is ready
        document.addEventListener('DOMContentLoaded', function() {
            // Get the overtime modal element
            const overtimeModal = document.getElementById('addOvertimeModal');
            
            // Add event listener for when the modal is shown
            if (overtimeModal) {
                overtimeModal.addEventListener('shown.bs.modal', function() {
                    // Get the activity select element inside this modal
                    const activitySelect = this.querySelector('select[name="activity_id"]');
                    
                    // Add change event listener to the activity select
                    if (activitySelect) {
                        activitySelect.addEventListener('change', function() {
                            updateMaxDays(this);
                        });
                        
                        // Also trigger the change event in case an activity is pre-selected
                        if (activitySelect.value) {
                            updateMaxDays(activitySelect);
                        }
                    }
                });
            }
